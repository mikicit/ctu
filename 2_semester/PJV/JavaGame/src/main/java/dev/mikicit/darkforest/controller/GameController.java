...
// Event Handlers
    ...
    private void updatePlayerPosition(double delta) {
        ...
        double path = player.getSpeed() * delta; // real distance traveled in time without reference to frame rate
        double pathBound = path + 1;

        if (input.contains("A") && player.getX() - pathBound > 0) {
        ...
        if (input.contains("D") && player.getX() + player.getWidth() + pathBound < tileMap.getMapWidth()) {
            int tileMinY = TileMap.convertPixelToTile(moveBox.getMinY());
            int tileMaxY = TileMap.convertPixelToTile(moveBox.getMaxY());
        ...
        if (input.contains("W") && player.getY() - pathBound > 0) {
        ...
        if (input.contains("S") && player.getY() + player.getHeight() + pathBound < tileMap.getMapHeight()) {
            int tileMinX = TileMap.convertPixelToTile(moveBox.getMinX());
